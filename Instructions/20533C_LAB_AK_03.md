...
4. On the Settings blade, ensure that **Use managed disk** is set to **Yes** and that **HQ-VNET** is selected as the **Virtual network**.
...
1. In the Edit Parameters window, ensure that the **Save passwords** check box is enabled and then click **Save**.

2. In the Deploy to Resource Group window, click **OK**.
...
4. In the Solution Explorer pane, right-click **ResDevLinuxDeploy**, click **Deploy**, and then click **20533C0301-LabRG**.

5. In the Deploy to Resource Group window, click **Edit Parameters**.

6. In the Edit Parameters window, in the vmName **Value** box, type **ResDevApp2**, and then click **Save**.
...
1. On MIA-CL1, right-click the Windows PowerShell icon on the taskbar and click **Run as Administrator**. When prompted by User Account Control, click **Yes**.
