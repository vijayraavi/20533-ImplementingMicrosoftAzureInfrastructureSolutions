...
1. In the Azure portal, on the Recovery Services vault blade, scroll down to the **MANAGE** section and click **Backup Infrastructure**. Then click **Protected Servers**.

2. On the **PROTECTED SERVERS** blade, click **Azure Backup Agent**.

3. On the **Protected Servers (Azure Backup Agent)** blade, verify that there is an entry referencing MIA-CL1. Click the **MIA-CL1.** link.

4. On the **MIA-CL1.** blade, click **Delete**.

5. On the **Delete** blade, in the **TYPE THE SERVER NAME** text box, type **MIA-CL1.** and click Delete. This should de-register the server. 

> **Note**: The entry **MIA-CL1.** is case-sensitive and includes the trailing period. If the Delete (de-register) does not complete, the Recovery Services Vault cannot be deleted later. If this happens, continue with the next exercise.

6. Scroll back left to the Recovery Services Vault blade and click **Overview**. You may have to scroll up also.

7. Click **Delete**. When prompted to confirm, click **Yes**. 

> **Note**: Wait for the vault to be deleted. If the Delete (de-register) you invoked in step 5 did not complete, the Recovery Services Vault cannot be deleted. If this happens, continue with the next exercise.
