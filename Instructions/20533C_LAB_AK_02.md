...
10.  Type the following command, and then press Enter:
  ...
  ```
  $rootCert=New-SelfSignedCertificate -KeySpec KeyExchange -Subject 'CN=AdatumRootCertificate' -KeyExportPolicy Exportable -CertStoreLocation Cert:\CurrentUser\My\ -FriendlyName 'AdatumRootCertificate'
  ```

11. Type the following command, and then press Enter:

  ```
  [System.Convert]::ToBase64String($rootCert.GetPublicKey())
  ```

12. Select the the output of the previous command and press Enter.

13. Switch back to Internet Explorer.

14. Paste the copied text into the **PUBLIC CERTIFICATE DATA** textbox on the **gatewayARM - Point-to-site configuration** blade of the Azure portal. 

15. In the **NAME** text box to the left, type **AdatumRootCertificate** and click **Save**.

16. Switch back to the Windows PowerShell window.

17. At the Windows PowerShell prompt, type the following command, and then press Enter:

  ```
  New-SelfSignedCertificate -Subject 'CN=AdatumClientCertificate' -KeyExportPolicy Exportable -KeySpec KeyExchange -NotAfter (Get-Date).AddMonths(96) -CertStoreLocation Cert:\CurrentUser\My\ -FriendlyName 'AdatumClientCertificate' -Signer $rootCert
  ```

18. At the Windows PowerShell prompt, run the following command:

  ```
  Get-ChildItem Cert:\CurrentUser\My\
  ```

19. Verify that both the **AdatumClientCertificate** and **AdatumRootCertificate** display in the Personal store.
