...
1. On **MIA-CL1**, open a Windows PowerShell Integrated Scripting Environment (ISE) window, open the script  **D:\Labfiles\Lab03\Starter\CreateRmVM.ps1** **and review its content.**
...
6. In the Solution Explorer, right-click **ResDevLinuxDeploy**, and then start a new deployment process for the first virtual machine.
...
8. View the contents of the Azuredeploy.parameters.json file to see that the parameters that you entered have been saved in this file.

9. Start another deployment process by using the deployment that you used for the first virtual machine. 

10. Repeat step 4, changing only the following parameter:
...
11. Close the solution, but leave Visual Studio open.
