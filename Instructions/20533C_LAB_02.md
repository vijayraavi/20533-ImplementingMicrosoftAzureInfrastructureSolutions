...
3. On MIA-CL1, open a Command Prompt window with elevated privileges, and change the current directory to **C:\Program Files (x86)\Windows Kits\10\bin\x64**.
...
5. Open the Certificates console targeting the certificate store of the current user.

6. Export the newly created certificate without its private key and in the **Base-64 encoded X.509 (.CER)** format to the **D:\Labfiles\Lab02\Starter\AdatumRootCertificate.cer** file.

7. Open **D:\Labfiles\Lab02\Starter\AdatumRootCertificate.cer** in Notepad.

8. Remove all line breaks between individual lines between the **-----BEGIN CERTIFICATE------** and **------END CERTIFICATE------** lines, so the certificate content is one continuous line.

9. Copy the single line between the **-----BEGIN CERTIFICATE------** and **------END CERTIFICATE------** lines
...
14. Switch back to the Certificates console, verify that both the **AdatumClientCertificate** and ** AdatumRootCertificate** display in the Personal store.
...
3. Verify the VPN connection by initiating an RDP session to the private IP address of ARMSrv2 Azure virtual machine. Verify that you can successfully log on to ARMSrv2.
