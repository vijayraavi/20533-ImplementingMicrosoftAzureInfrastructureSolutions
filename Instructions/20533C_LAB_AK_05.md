...
3. On the web app blade, in the **APP DEPLOYMENT** section, click **Deployment slots**.
...
1. In the Azure portal, in the web app that you created in the first task, on the web app blade, in the **APP DEPLOYMENT** section click **Deployment credentials**.
...
2. On the web app blade, in the **APP DEPLOYMENT** section, click **Deployment slots**.
...
6. At the Windows PowerShell prompt, type the following command (where the _'webappname2'_ is a unique name that you will assign to a new web app you will create in the _'SecondLocation'_ in the next step), and then press Enter:
  ```
  Test-AzureRmDnsAvailability -DomainNameLabel 'webapp' -Location 'SecondLocation'
  ```
...
20. In the **Downloads** folder, select the **_WebAppName_2.PublishSettings** file, and then click **Open**.
...
> **Note:** If the record has not changed, this might be related to DNS caching. Wait about 1 minute and re-run the **nslookup** command.
