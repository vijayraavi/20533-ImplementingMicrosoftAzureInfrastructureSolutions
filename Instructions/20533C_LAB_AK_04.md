...
4. In the  **New** blade, click **Compute**.

5. In the  **Compute** blade, click **Windows Server 2012 R2 Datacenter**.
...
9. In the ResDevWebLB blade, click **Health Probes**, and then in the  **Health Probes** blade, click **Add**.
...
6. In the  **Add inbound security rules** blade, along the top click **Advanced** specify the following settings:
...
2. Close this blade to return to the **ResDevWebRG** blade.
...
1. Repeat the steps 4 and 5 to attach **two** more new data disks. Note that with current VM size (Standard A1), there is a limit of 2 data disks per VM.
...
25. From the taskbar of ResDevWebVM1, open File Explorer, and then verify that there is a new drive F with 2 TB of available disk space.
