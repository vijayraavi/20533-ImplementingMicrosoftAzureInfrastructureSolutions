...
3. In the Azure portal, in the hub menu, click **More services** and, in the service menu, click **Virtual machines**.
...
15. In Internet Explorer, navigate to https://docs.docker.com/docker-for-windows/install/#download-docker-for-windows
...
5. In the Remote Desktop session to 20533D0701-vm0, start Internet Explorer and browse to https://portal.azure.com. When prompted, sign in by using the Microsoft account that is the Service Administrator of your Azure subscription. 
...
12. Identify the values appearing in the **Sku** column in the output. Verify that **16.04.0-LTS** is available. If that is not the case, replace **16.04.0-LTS** with one of avaialble SKUs in step 14.
...
16. From the **Administrator: Command Prompt** window, type the following, replacing `Azure-subscription-Id` with the value of your Azure subscription ID you copied earlier in this task, and replacing the `Azure-region` with the name of the Azure region you selected when running **Add-20533DEnvironment** at the beginning of this module, and then press Enter (make sure to use the lower case characters when specifying the virtual machine name): 
  ```
  docker-machine create --driver azure --azure-ssh-user student --azure-subscription-id "Azure-subscrption-id" --azure-open-port 80 --azure-image "Canonical:UbuntuServer:16.04.0-LTS:latest" --azure-location "Azure-region" --azure-resource-group "20533D0702-LabRG" --azure-availability-set "20533D0702-avset" --azure-static-public-ip "20533d0702-vm0"
  ```

17. The **Administrator: Command Prompt** window will display the message asking you to use a web browser to open the page https://aka.ms/devicelogin and enter the code provided at the end of the message. Start Intrenet Explorer and browse to https://aka.ms/devicelogin. 
...
26. To verify connectivity to the target Docker Azure VM, in the **Administrator: Command Prompt** window, type the following, replacing `IP-address` with the IP address you identified in the previous step and then press Enter: 
  ```
...
2. Review the output returned by the docker-machine env command. Next, in the **Administrator: Command Prompt** window, type the following, and then press Enter: 
  ```
...
4. In Notepad, type the following text, click **File** and click **Save**:
  ```
  wordpress:
    image: wordpress
    links:
...
1. In the Remote Desktop session to 20533D0701-vm0, in the Azure portal in the Internet Explorer window, click **+ New**.
...
1. In the Remote Desktop session to 20533D0701-vm0, in the Azure portal in the Internet Explorer window, click **More services**, and in the service menu, click **Container registries**.
