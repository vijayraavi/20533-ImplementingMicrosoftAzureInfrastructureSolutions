...
1. Open an Internet Explorer window and browse to https://myapps.microsoft.com. When prompted, sign in by using specify the full user name (including the @_domain name_.onmicrosoft.com suffix) of the Mario Ledford's account and the corresponding password **Pa55w.rd**. 
...
4. Switch back to the Internet Explorer window, click **Skype** and, when prompted, install the Access Panel Extension add-in with the default settings and enable the add-in once the installation completes.
